...
> ofertante), não pela classificação — logo tipos flexíveis já caem no destinatário certo.
...
## Aba `Transferencias` (comprovante)
A transferência agrupa N pagamentos (1 por despesa). Colunas relevantes ao **comprovante** (anexo no Google Drive):

| Coluna | Tipo | Descrição |
|--------|------|-----------|
| comprovante_file_id | string | id do arquivo no Drive (vazio = sem anexo) |
