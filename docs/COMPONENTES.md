...
| `despesa-table` | `.despesas`, `.categorias`, `.participantes`; eventos `abrir` (clique na linha), `editar`, `remover` | Tabela **full-width** e fluida; colunas Item, **Classificação** e **Subclassificação**, **Ofertante** (`ofertanteNome`, contato/equipe) e **Empresa** (`fornecedor_id`), Adicionado/Editado por + **Pago**, **Pagamento** (total), **Distribuição** e **Responsabilidade**. Reusa `ui-data-table` + `category-badge`. |
| `despesa-detail` | `.despesa`, `.categorias`; evento `fechar` | **Banner (modal)**. Despesa de **oferta** (`preco_id`): Item/Valor/Ofertante/Empresa **read-only** (resumo) + Data; se ofertante é **equipe**, edita **Recebido por integrante** (`split-editor`). Sempre editáveis: **Subclassificação**/observação/**Pago** + **Pagamento** (quem pagou) e **Responsabilidade** (`split-editor`). Despesa **legada** (sem `preco_id`): abas Material/Serviço + item + valor editáveis. **Regras** (`ui-alert`): Σ pagamentos ≤ valor; Σ % ≤ 100; Σ recebidos ≤ valor. |
...
| `despesa-split.js` | `parseLista`, `totalPago`, `distribuicao`, `rotuloOrigem`, `acerto` | Helpers puros da divisão + algoritmo de **acerto** "quem deve a quem" (independente de ofertante/recebidos). |
