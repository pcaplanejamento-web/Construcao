...
| `ui-data-table` | `.columns` (`{chave,titulo,formato?,alinhar?,largura?}`; `largura` = min-width opcional p/ coluna, ex.: "200px"), `.rows`, `.acoes`; attrs `empty-text`, `fluido` (células proporcionais/quebram), `clicavel` (linha clicável) | `acao` ({acao,linha}), `linha` ({linha}) | Tabela genérica orientada a dados. |
...
| `despesa-table` | `.despesas`, `.categorias`, `.participantes`; eventos `abrir` (clique na linha), `editar`, `remover` | Tabela **full-width** e fluida; colunas Item (largura mín. 200px p/ não quebrar textos longos), **Classificação** e **Subclassificação**, **Ofertante** (`ofertanteNome`) e **Empresa**, Adicionado/Editado por + **Status** (A pagar/Em pagamento/Pago — `statusPagamento`), **Pagamento** (quem pagou, total), **Distribuição** e **Responsabilidade**. Reusa `ui-data-table` + `category-badge`. |
