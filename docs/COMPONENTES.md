...
| `obra-detail-view` | attr `id` (rota); — | **Coração do tempo real**: KPIs + `ui-tabs` (**Gráficos** / **Despesas** / **Participantes** / **Responsáveis** / **Orçamentos** / **Equipes** / **Fornecedores** / **Pagamentos**); dashboard + despesas (otimista + cache). Aba **Despesas**: "+ Registrar Despesa" (`cotacao-despesa-form` obra-fixa). Aba **Orçamentos**: "+ Novo orçamento" abre `orcamento-form` com a obra **travada** + grade dos orçamentos da obra. Aba **Fornecedores**: empresas usadas na obra + Nº/Total/Recebido/**Saldo a receber** (`balancos.porFornecedor`; clique → fornecedor). Aba **Pagamentos**: tabela dos pagamentos da obra (Data/Valor/Pagou/Recebedor/Nº despesas) com "+ Registrar pagamento" (`pagamento-form`, multi-despesa), **Repassar** (`repasse-form`) e excluir (`removerPagamentoV2`); + tabela de **Repasses**. Na aba **Despesas**, ao SELECIONAR despesas (barra de seleção): **Lançar pagamento** (abre `pagamento-form` com as selecionadas; avisa/ignora as que já têm pagamento) e **Definir responsabilidade** (modal com `split-editor` aplicado às selecionadas). |
...
| `pagamento-form` | `.obra`; eventos `salvo`, `fechar` | **Banner (modal)** p/ registrar um pagamento que cobre **VÁRIAS despesas** (entidade Pagamentos): lista as despesas da obra com saldo (checkbox + alocação por despesa, default = resto, total ao vivo), **Quem pagou** (participante), **Recebedor** (contato OU equipe/grupo), **Empresa** opcional e data → `dataStore.lancarPagamentoMulti`. |
...
| `pagamento-util.js` | [features/pagamentos/pagamento-util.js](../src/features/pagamentos/pagamento-util.js) | `previaPagamentoHtml(p)` — conteúdo do card de prévia do pagamento (esverdeado; espelha `previaOfertaHtml`). `abrirPagamento(p)` — **banner** (`ui-modal`) com dados completos: valor/data/pagou/recebedor/empresa/obra, **despesas cobertas** e **repasses como cards**. Reusado no `despesa-detail` e na `pagamentos-view`. Também: `nomeContato/nomeEquipe/nomeChavePart/nomeRecebedor`. |
| `pagamentos-view` | rota `/pagamentos` (menu lateral) | Lista TODOS os pagamentos como cards padronizados (esverdeados, `previaPagamentoHtml`); clique → `abrirPagamento` (mesmo banner das despesas). |
| `pagamento-form` (multi-despesa) | + prop `.despesasSelecionadas` | Quando recebe despesas selecionadas, lista SÓ essas, já marcadas (usado pela ação em massa "Lançar pagamento"). |
| `despesa-detail` | `.despesa`, `.categorias`; evento `fechar` | **Banner (modal)**. Despesa de **oferta** (`preco_id`): mostra a **oferta de origem** pelo **mesmo card de prévia** (`previaOfertaHtml`); **clicar abre** o **banner único da oferta em só-leitura** (`abrirOferta(oferta,{somenteLeitura:true})`). Para editar, edita-se a OFERTA na origem. + Data. **Sem editor de subclassificação** (vem do item). Seção **Pagamentos**: badge de **status** + Pago/Resto, os pagamentos como **cards esverdeados** (`previaPagamentoHtml`; clique → `abrirPagamento`; ✕ exclui e **desvincula** via `removerPagamentoV2`). Se já houver pagamento, o form de lançar fica **oculto** (exclua antes de relançar). Mini-form **Lançar pagamento** (**Quem pagou** = participante + valor + data + `split-editor` de integrantes p/ equipe) → `dataStore.lancarPagamento`. O "quem pagou quanto" é **derivado das levas**. Também editável: observação e **Responsabilidade** (`split-editor`). Legada (sem `preco_id`): item/valor editáveis. **Regras** (`ui-alert`): quem pagou obrigatório; Σ % ≤ 100; leva ≤ resto; Σ distribuição ≤ leva. |
