...
| `equipe-detail-view` | attr `id` (rota `#/equipes/:id`) | Cabeçalho + seções **Obras vinculadas**, **Membros** e **Dados** (recebimentos: total/pago/**saldo a receber** da equipe + **Recebido** por integrante, somado das `distribuicao` das levas). Salva via `dataStore.atualizarEquipe`. |
