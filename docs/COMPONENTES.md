...
| `obra-detail-view` | attr `id` (rota); — | **Coração do tempo real**: KPIs + `ui-tabs` (**Gráficos** / **Despesas** / **Participantes** / **Responsáveis** / **Orçamentos** / **Equipes** / **Fornecedores**); dashboard + despesas (otimista + cache). Aba **Fornecedores**: empresas usadas na obra + Nº/Total/Pago/**Saldo a receber** (`restosESaldos.porFornecedor`; clique → fornecedor). |
| `obra-participantes` | attr `obra-id`, `modo` (participantes\|responsaveis) | Aba de participantes com **acerto de contas**: colunas Pago/Devido/**Saldo** + painel **"quem deve a quem"** (`despesa-split.acerto`) **+ Restos a pagar / Saldo a receber** (`restosESaldos`, dos pagamentos parciais). modo `participantes`: todos + adicionar/remover; modo `responsaveis`: só os marcados. |
...
| `despesa-detail` | `.despesa`, `.categorias`; evento `fechar` | **Banner (modal)**. Despesa de **oferta** (`preco_id`): Item/Valor/Ofertante/Empresa **read-only** + Data. Seção **Pagamentos**: badge de **status** + Pago/Resto, lista de **lançamentos (levas)** com remover, e mini-form **Lançar pagamento** (data + valor + `split-editor` de integrantes p/ equipe) → `dataStore.lancarPagamento`. Também editáveis: **Recebido planejado** (equipe), **Subclassificação**/observação + **Pagamento** (quem pagou) e **Responsabilidade** (`split-editor`). Legada (sem `preco_id`): item/valor editáveis. **Regras** (`ui-alert`): Σ pagamentos ≤ valor; Σ % ≤ 100; Σ recebidos ≤ valor; leva ≤ resto; Σ distribuição ≤ leva. |
...
| `despesa-split.js` | `parseLista`, `totalPago`, `distribuicao`, `rotuloOrigem`, `acerto`, `totalRealizado`, `restoDespesa`, `statusPagamento`, `restosESaldos` | Helpers puros: divisão + **acerto** "quem deve a quem"; e **pagamentos parciais** (status derivado, total realizado/resto, `restosESaldos` → restos a pagar/saldo a receber por chave e por fornecedor). |
...
| `fornecedor-detail-view` | attr `id` (rota `#/fornecedores/:id`) | Página do fornecedor: cabeçalho + `ui-tabs` com **Contatos**, **Ofertas**, **Orçamentos** e **Dados** (por obra: Total/Pago/**Saldo a receber**; clique → obra). |
...
| `contato-detail-view` | attr `id` (rota `#/contatos/:id`) | Página do contato: `ui-tabs` — **Obras**, **Fornecedores** (se Vendedor), **Equipes**, **Ofertas**, **Orçamentos** e **Dados** (por obra: **Saldo a receber** + **Restos a pagar**; clique → obra). |
...
| `cotacao-despesa-form` | `.cotacao`+`.preco` (oferta-fixa) **ou** `.obraFixaId` (obra-fixa); eventos `registrado`, `fechar` | Banner de **registro de oferta como despesa** (modo duplo). **oferta-fixa** (da cotação/orçamento): escolhe a **obra**. **obra-fixa** (botão "+ Registrar oferta" na obra): abas Material/Serviço + `ui-select` de **ofertas ainda não registradas**. Ambos: **Subclassificação** + **Responsabilidade** (`split-editor` %) e, se o ofertante é **equipe**, **Recebido por integrante** (`split-editor` R$). Chama `dataStore.registrarDespesaOferta` (marca a oferta + fecha a cotação). |
...
| `equipe-detail-view` | attr `id` (rota `#/equipes/:id`) | Cabeçalho + seções **Obras vinculadas**, **Membros** e **Dados** (recebimentos: total/pago/saldo da equipe + por integrante Planejado/Recebido/**Saldo a receber**). Salva via `dataStore.atualizarEquipe`. |
