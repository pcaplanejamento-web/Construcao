...
| `ui-data-table` | `.columns` (`{chave,titulo,formato?,alinhar?,largura?,secundaria?,`**`moeda?`**`,`**`valorNum?(linha)`**`}`; `secundaria`=some no mobile ≤820px; **`moeda:true`**=entra no somatório; **`valorNum`**=número a somar p/ total derivado, default `Number(linha[chave])`), `.rows`, `.acoes`; attrs `empty-text`, `fluido`, `clicavel`, **`excluir-massa`** | `acao`, `linha`, **`selecao`** ({linhas}), **`excluir-massa`** ({linhas}) | **Data-grid** (não mais passivo): **coluna de seleção fixa à esquerda** (+ Selecionar Todos), **cabeçalho clicável** abre `ui-coluna-menu` (ordena/filtra por Valores — estado interno que **persiste** entre re-renders), **busca global** embutida (`ui-busca` no canto sup. direito, sobreposta ao cabeçalho — atualiza só o corpo p/ preservar foco/texto), **linha de Totais** (`<tfoot>` sticky-bottom, soma das colunas `moeda` das linhas visíveis), **cabeçalho sticky-top**, **barra de seleção** com a SOMA dos selecionados + "Excluir selecionadas" (se attr `excluir-massa`). `.wrap` rolável (`max-height:70vh`) → totais e scroll fixos na base. Células `.dir` em Space Grotesk 700; avatar na 1ª coluna via `formato`. |
...
| `ui-busca` | attr `placeholder`; método `definir(texto)` | `buscar` ({texto}) | Botão de **pesquisa** (ícone `busca`) que expande o campo da **direita p/ a esquerda**, sobreposto (overlay) ao conteúdo à esquerda **sem deslocar nada**. Componente ÚNICO reusável; usado de forma padrão pelo `ui-data-table`. Fecha ao perder foco vazio / Esc. |
