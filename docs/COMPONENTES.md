...
| `preco-form` | `.cotacao`/`.cotacaoId`, `.orcamento`, `.preco`; eventos `salvo`, `fechar` | Banner **ÚNICO "Criar oferta"** (reusado em cotação, orçamento e aba Ofertas). Escolhe o **Item** (define classificação/subclassificação, exibidas ao vivo) → **Ofertante** (contato ou grupo) + **Fornecedor** (auto-preenchido pelo contato) + Quantidade, Valor unitário, Valor unit. com desconto, **Prazo (obrigatório)**, obs. Regras: **Material** → fornecedor obrigatório; **Serviço** → ofertante obrigatório. **Edição usa o MESMO form** (`.preco`) com TODOS os campos editáveis e **pré-preenchidos** (inclusive o item). Ofertas **legadas** (sem `item_id`/`quantidade` próprios) pré-preenchem item/quantidade a partir da **cotação** vinculada (e incluem o item na lista mesmo se inativo). Item travado só ao **criar a partir de uma cotação**; ofertante+fornecedor travados só ao **criar dentro de um orçamento**. Chama `dataStore.criarOferta`/`atualizarOferta`. |
