...
| `app-header` | evento `toggle-sidebar` | Cabeçalho persistente (sticky). **Marca-bloco** (largura = sidebar aberta, 200px): **logo** `src/assets/dattaobra.png` (transparente) + texto **"Dattaobra"** (link → `#/obras`) + **botão sanduíche** encostado no limite direito da sidebar (recolhe no desktop / drawer no mobile); o logo alinha com a coluna de ícones da sidebar. À direita: **alternador de tema** (sol/lua), chip do usuário → `#/perfil`, Sair. Ícones via `ui-icon`. |
