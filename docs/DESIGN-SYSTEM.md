...
- **Dock flutuante (`app-shell`)**: atalhos rápidos num **dock flutuante estilo iOS** (pílula de vidro: `backdrop-filter: blur`, `border-radius: 26px`, `--sombra-lg`, centralizado, flutua acima do conteúdo), **em MOBILE e DESKTOP** (só autenticado; some no login e no link público). Itens: Contatos · Fornecedores · Obras · Transferências · Orçamentos — cada tile com **acento de cor** próprio (`--cor-info/roxo/primaria/aviso/sucesso` via vars `--bi`/`--bi-suave`); item ativo com anel da cor. `:host([com-barra]) main` ganha `padding-bottom` p/ o conteúdo não ficar atrás. Gating via atributo `com-barra` no host. **Auto-ocultar (iOS 26)**: ao **rolar p/ baixo** o dock desliza e some (`.oculto` = `translateY(180%)` + `opacity:0`); **reaparece ao subir** ou no topo (listener de scroll no `#outlet`).
