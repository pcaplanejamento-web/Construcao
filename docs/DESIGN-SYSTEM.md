...
- **iOS / barra transparente (iOS 26)**: `index.html` usa `viewport-fit=cover` + metas `apple-mobile-web-app-*` (status bar translúcida). O `app-shell` usa **`100dvh`** (dynamic viewport — acompanha a barra do Safari que some/aparece; **header sempre fixo no topo**), e **`env(safe-area-inset-*)`** dá folga ao header (topo/laterais), ao `<main>` (base) e ao drawer/`ui-modal`, para nada ficar sob o notch / home indicator / barra flutuante.
...
- **`ui-data-table`**: ≤600px cada **linha vira um CARD empilhado** — `thead` some, `tr` vira card (borda+sombra+raio), cada `td` é `display:flex` "RÓTULO ↔ valor" (rótulo via `::before { content: attr(data-label) }`, divisória sutil entre campos, feedback de toque no `:active`). **Mostra só as colunas PRINCIPAIS** (as `.sec` continuam ocultas — card conciso; o toque na linha abre o detalhe completo). Seleção no canto + ações em linha cheia; totais (`tfoot`) viram card. Sem rolagem horizontal, sem hover. 601–820px: tabela rola na horizontal, escondendo `.sec`.
