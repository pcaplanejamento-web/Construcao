...
- **Barra do navegador TRANSLÚCIDA (Safari iOS 26 / Liquid Glass)**: a barra é **sempre translúcida**; o Safari 26 **ignora `theme-color`** (um valor fixo só atrapalha — vira barra estática), então o app **não define `theme-color`** (removido do `<meta>`, do anti-FOUC e do `theme.js`). O **tom** da barra é amostrado do conteúdo perto das bordas: `background-color`/`backdrop-filter` de elementos **`fixed`/`sticky`** (ex.: o `app-header` com vidro tinge a barra de cima) e, como fallback, a **cor de fundo do `html`/`body`** (por isso o `html` tem `background-color: var(--cor-fundo)` — sem cor no root, em `scrollY=0` o Safari cairia para **branco**). Requisitos: `viewport-fit=cover` + cor no root + `env(safe-area-inset-*)` + `apple-mobile-web-app-status-bar-style: black-translucent`. Trade-off: Chrome (Android) deixa de tingir a barra (usa o padrão).
