/**
 * <ui-select> — Lista suspensa PESQUISÁVEL (combobox). Usado em TODO dropdown do
 * sistema: campo com **busca** (filtra por rótulo), teclado (↑/↓/Enter/Esc) e
 * alvos de toque ≥44px. O painel **expande EM FLUXO** (empurra o conteúdo e faz
 * `scrollIntoView`) — assim não recorta dentro de modais (que têm
 * `overflow`/`backdrop-filter`).
 *
 * API (retrocompatível com o antigo ui-select nativo):
 *   Atributos: `label`, `name`, `value`, `placeholder`, `error`, e o opcional
 *     `criar` (rótulo) — quando presente, mostra a linha "+ <rótulo>" no fim da
 *     lista e emite o evento **`criar`** (o pai abre o cadastro correspondente).
 *   Propriedade: `.options = [{ value, label }]`, `.value` (lê/escreve).
 *   Eventos: **`change`** ({ value, name }) e **`criar`**.
 */
...
class UiSelect extends BaseElement {
  static get observedAttributes() {
    return ["label", "placeholder", "criar", "value"];
  }
  ...
  aposRender() {
    ...
    if (!inp) return;
    // Valor inicial vindo do ATRIBUTO (ex.: <ui-select value="ativa">).
    if (this._value == null) {
    ...
    if (chev) chev.addEventListener("mousedown", (e) => { e.preventDefault(); if (this._aberto) inp.blur(); else inp.focus(); });
    const criar = this.$(".criar");
    if (criar) criar.addEventListener("mousedown", (e) => { e.preventDefault(); clearTimeout(this._blurT); this._fechar(); this.emitir("criar"); });
    this._pintarLista();
  }
  ...
  _pintarLista() {
    ...
    box.innerHTML = cs.length
      ? cs.map((o, i) => `<button class="op ${String(o.value) === String(this.value) ? "sel" : ""} ${i === this._hi ? "hi" : ""}" type="button" data-v="${esc(o.value)}">${esc(o.label)}</button>`).join("")
