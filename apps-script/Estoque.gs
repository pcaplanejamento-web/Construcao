...
function estoqueCriarMovimento(data, sessao) {
  ...
  _obraAcessivel(obraId, usuarioId); // valida acesso à obra
  ...
  if (acao === "manual") {
    const item = _itemParaObra(itemId, usuarioId, _refsAcessiveis(usuarioId).refI); // valida + herda classificação/subclasse (item 16)
  ...
  if (acao === "transferir") {
    ...
    const item = _itemParaObra(itemId, usuarioId, _refsAcessiveis(usuarioId).refI);
