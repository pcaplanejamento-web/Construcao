...
function fornecedoresIncorporar(data, sessao) {
  ...
  return comLock(function () {
    // Dedupe: cópia pessoal EXATA (origem_id) OU equivalente (nome) → devolve.
    const nk = String(origem.nome || "").trim().toLowerCase();
    const jaMinha = repoEncontrar(SCHEMA.FORNECEDORES, function (f) {
      if (String(f.usuario_id) !== String(sessao.usuario_id) || !_fornecedorAtivo(f)) return false;
      if (String(f.origem_id || "") === String(origem.id)) return true;
      return String(f.nome || "").trim().toLowerCase() === nk;
    });
